...
/**
 * mh3685 Homework 5 Dijkstra.java
 * This class is a response to Homework 5, Programming Problem #3. Student code implements a method to
 * compute the Euclidean distance between two input vertices and set the weight for the corresponding
 * edge on the graph, a method to implement Dijkstra's algorithm starting at a given city, and a method
 * to get the shortest path from one city to another after Dijkstra's algorithm has been performed.
 * 
 * The rest of the code has been left unmodified.
 * @author Mari Husain (of the student code)
...
public class Dijkstra {
  ...
  public void computeAllEuclideanDistances() {
	  // iterate through all cities, get all their directed edges, and set weights
        for(String name: vertexNames.keySet()) {
        	for(Edge edge: vertexNames.get(name).adjacentEdges) {
  ...
  public void doDijkstra(String s) {
        Vertex startPoint = vertexNames.get(s);
        LinkedList<Vertex> unknownVertices = new LinkedList<Vertex>(vertexNames.values());
        
        for(Vertex vertex: unknownVertices) {
        	vertex.distance = Double.MAX_VALUE;
        	vertex.known = false;
        	vertex.prev = null;
        }
        
        startPoint.distance = 0; // set starting point distance to zero
        startPoint.known = true;
        unknownVertices.remove(startPoint);
        
        Vertex currentCity = startPoint;
        
        while(!unknownVertices.isEmpty()) {
        	// for all cities connected to the current city, update distances, if necessary
        	for(Edge edge: currentCity.adjacentEdges) {
        		double newTravelDistance = currentCity.distance + edge.distance;
        		if(!edge.target.known && newTravelDistance < edge.target.distance) {
        			edge.target.distance = newTravelDistance;
        			edge.target.prev = currentCity;
        		}
        	}
        	
        	// choose the smallest distance
        	Vertex smallestDistanceCity = unknownVertices.getFirst();
        	for(Vertex vertex: unknownVertices) {
        		if(vertex.distance < smallestDistanceCity.distance) {
        			smallestDistanceCity = vertex;
        		}
        	}
        	
        	// mark smallest distance city as known and set it to current city
        	smallestDistanceCity.known = true;
        	unknownVertices.remove(smallestDistanceCity);
        	currentCity = smallestDistanceCity;
        }
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    Vertex currentVertex = vertexNames.get(t);
    while(currentVertex.prev != null) {
    	Vertex previousCity = currentVertex.prev;
    	for(Edge edge: currentVertex.adjacentEdges) {
    		if(edge.target.name.equals(previousCity.name)) {
    			path.add(edge);
    		}
    	}
